...
class ReportSplitter(tk.Tk):
    ...
    def _build_ui(self):
        hdr = tk.Frame(self, bg=PRIMARY_DARK, pady=10, padx=12)
        hdr.pack(fill="x")
        if self._app_icon is not None:
            self._header_icon = self._app_icon.subsample(2, 2)
            tk.Label(hdr, image=self._header_icon, bg=PRIMARY_DARK).pack(
                side="left", padx=(0, 10))
        tk.Label(hdr, text="Hilton High School",
                 bg=PRIMARY_DARK, fg="white",
                 font=("Segoe UI", 14, "bold")).pack(side="left")
        self._b(hdr, "⚙  Settings", self._open_secret_settings,
    ...
    def _gen_right(self, parent):
        ...
        appearance = tk.Frame(col, bg=CARD)
        appearance.pack(fill="x", pady=(0, 10))
        tk.Label(appearance, text="Appearance", bg=CARD,
        ...
        self.theme_value_label.pack(side="right", padx=(4, 6))
        self._b(appearance, "Change Theme", self._open_theme_picker,
                bg=BTN_BG, padx=8, pady=3).pack(side="right")
    ...
    def _build_tab_paycodes(self):
        ...
        tk.Entry(meta, textvariable=self.pc_search_var, font=FONT,
                 bd=1, relief="solid", bg=ENTRY_BG, width=28).pack(side="right", padx=(0, 6))
    ...
    def _build_tab_defaulters(self):
        ...
        tk.Entry(ar, textvariable=self.df_manual_var, font=FONT,
                 bd=1, relief="solid", bg=ENTRY_BG, width=32).pack(side="left", padx=(8, 4))
